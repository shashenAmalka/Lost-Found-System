...
import { Users, Shield, ShieldAlert, ShieldOff, AlertTriangle, Search, Ban, Undo2, MinusCircle, Loader2, FileText } from 'lucide-react'
...
export default function UserModerationPanel() {
    ...
    const [expandedAppeals, setExpandedAppeals] = useState({})
    ...
    const handleAppealDecision = (result, fallbackUserId = null) => {
        fetchData()
        const resolvedUserId = result?.userId || result?.appeal?.userId?._id || result?.appeal?.userId || fallbackUserId
        if (!resolvedUserId) return
        fetchWarnings(resolvedUserId)
        fetchAppeals(resolvedUserId)
    }
    ...
    const fetchAppeals = async (userId) => {
        try {
            const res = await fetch(`/api/admin/appeals?status=all&userId=${userId}`, { credentials: 'include' })
            const data = await res.json()
            setExpandedAppeals(prev => ({ ...prev, [userId]: data.appeals || [] }))
        } catch {
        }
    }
    ...
    const toggleExpand = (userId) => {
        ...
        if (!expandedAppeals[userId]) fetchAppeals(userId)
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {appeals.length > 0 && (
                <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
                    ...
                    <div className="grid gap-3">
                        {appeals.map(a => (
                            <AppealReviewCard key={a._id} appeal={a} onDecision={(result) => handleAppealDecision(result)} />
                        ))}
                    </div>
                </div>
            )}
            ...
            <div className="relative">
                ...
                <input
                    className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 pl-10 text-sm text-[#1C2A59] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#F0A500]/50 transition-shadow shadow-sm"
                    placeholder="Search users by name, campus ID, or email..."
                    value={search}
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {paginatedUsers.length > 0 ? paginatedUsers.map(u => {
                                ...
                                return (
                                    <Fragment key={u._id}>
                                        <tr
                                            className="group cursor-pointer hover:bg-gray-50 transition-colors border-b border-gray-100"
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3">
                                                <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase" style={{ background: st.bg, color: st.color }}>
                                                    {st.label}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3">
                                                <div className="flex gap-0.5 items-center">
                                                    {[1, 2, 3].map(i => (
                                                        <div
                                                            ...
                                                            style={{ background: i <= (u.activeWarnings || u.warningCount || 0) ? '#ef4444' : '#E5E7EB' }}
                                                        />
                                                    ))}
                                            ...
                                            <td className="px-4 py-3">
                                                {u.hasPendingAppeal && (
                                                    <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-amber-100 text-amber-700">
                                                        Pending {u.appealCount ? `(${u.appealCount})` : ''}
                                                    </span>
                                                )}
                                            </td>
                                            <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                                                <div className="flex gap-1.5">
                                                    ...
                                                    {u.status === 'active' && (
                                                        <button
                                                            onClick={() => handleAction(u._id, 'restrict_limited', 'Admin action')}
                                                            title="Restrict (Limited)"
                                                    ...
                                                    {(u.status === 'limited' || u.status === 'active') && (
                                                        <button
                                                            onClick={() => handleAction(u._id, 'restrict_full', 'Admin action')}
                                                            title="Restrict (Full)"
                                                            disabled={actionLoading === u._id}
                                                    ...
                                                    {u.status !== 'active' && (
                                                        <button
                                                            onClick={() => handleAction(u._id, 'unrestrict')}
                                                            title="Unrestrict"
                                                            disabled={actionLoading === u._id}
                                                    ...
                                                    {(u.activeWarnings || u.warningCount || 0) > 0 && (
                                                        <button
                                                            onClick={() => handleAction(u._id, 'reduce_warning')}
                                                ...
                                                <td colSpan={7} className="p-4 bg-gray-50/50 border-b border-gray-100">
                                                    <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-4">
                                                        ...
                                                        <div className="space-y-4">
                                                                                    ...
                                                                                    <div key={w._id} className="flex items-start gap-2 p-2 rounded-lg bg-gray-50 border border-gray-100">
                                                                                        ...
                                                                                        <div className="flex-1 min-w-0">
                                                                                            <div className="flex items-center gap-1.5 mb-0.5">
                                                                                                <span className="text-[8px] font-bold uppercase px-1.5 py-0.5 rounded" style={{ background: `${sevColors[w.severity]}15`, color: sevColors[w.severity] }}>
                                                                                                    {w.severity}
                                                                                                </span>
                                                            ...
                                                            <div className="bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                                                                <p className="text-[9px] font-bold uppercase text-gray-400 mb-2 flex items-center gap-1.5">
                                                                    <FileText size={10} /> Appeals
                                                                </p>
                                                                {expandedAppeals[u._id] ? (
                                                                    expandedAppeals[u._id].length === 0 ? (
                                                                        <p className="text-[10px] text-gray-400 italic">No appeals recorded.</p>
                                                                    ) : (
                                                                        <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                                                                            {expandedAppeals[u._id].map(a => (
                                                                                <AppealReviewCard key={a._id} appeal={a} onDecision={(result) => handleAppealDecision(result, u._id)} />
                                                                            ))}
                                                                        </div>
                                                                    )
                                                                ) : (
