...
const OWNERSHIP_MIN_LENGTH = 50;
const OWNERSHIP_MAX_LENGTH = 1000;

/**
 * Validate ownership explanation length
 * @param {string} text - Ownership explanation text
 * @returns {object} { valid: boolean, error?: string }
 */
export function validateOwnershipExplanation(text) {
  if (!text || !text.trim()) {
    return { valid: false, error: 'Ownership explanation is required' };
  }

  const trimmed = text.trim();

  if (trimmed.length < OWNERSHIP_MIN_LENGTH) {
    return {
      valid: false,
      error: `Ownership explanation must be at least ${OWNERSHIP_MIN_LENGTH} characters. Please describe unique marks, contents, or purchase details to prove ownership.`
    };
  }

  if (trimmed.length > OWNERSHIP_MAX_LENGTH) {
    return {
      valid: false,
      error: `Ownership explanation cannot exceed ${OWNERSHIP_MAX_LENGTH} characters`
    };
  }
...
/**
 * Validate that at least one strong piece of evidence is provided.
 * Strong evidence = linked lost report OR hidden details OR proof URL.
 * @param {object} form - { lostItemId, hiddenDetails, proofUrl }
 * @returns {object} { valid: boolean, error?: string }
 */
export function validateClaimEvidence(form) {
  const hasLinkedReport = !!(form.lostItemId && form.lostItemId.trim());
  const hasHiddenDetails = !!(form.hiddenDetails && form.hiddenDetails.trim());
  const hasProofUrl = !!(form.proofUrl && form.proofUrl.trim());

  if (!hasLinkedReport && !hasHiddenDetails && !hasProofUrl) {
    return {
      valid: false,
      error: 'At least one strong evidence is required: link a lost report, provide hidden identifying details, or add a proof URL.'
    };
  }
...
/** Ownership explanation limits (exported for UI character counters) */
export const CLAIM_LIMITS = {
  OWNERSHIP_MIN: OWNERSHIP_MIN_LENGTH,
  OWNERSHIP_MAX: OWNERSHIP_MAX_LENGTH,
};
