...
export default function MyLostItemsPage() {
    ...
    useEffect(() => {
        ...
        const fetchItems = async () => {
            try {
                const res = await fetch('/api/lost-items?page=1&limit=1000', { credentials: 'include' })
                const data = await res.json()
                const myItems = (data.items || []).filter(item => item.postedBy?.toString() === user.id)
                setItems(myItems)
            } catch {
                setItems([])
            } finally {
    ...
    const filteredItems = useMemo(() => {
        ...
        return sortedItems.filter((item) => {
            const matchesQuery = !query || [item.title, item.description, item.possibleLocation, item.category, item.keywords]
                .filter(Boolean)
                .some((value) => String(value).toLowerCase().includes(query))

            const matchesCategory = !filters.category || item.category === filters.category

            const itemLocation = String(item.possibleLocation || '').toLowerCase()
    ...
    const handleDelete = async (itemId) => {
        ...
        const confirmed = window.confirm('Delete this lost item report?')
        if (!confirmed) return
    ...
    const handleEdit = (item) => {
        ...
        setActionError('')
        router.push(`/lost-items/${item._id}/edit?returnTo=/my-lost-items`)
    }
        ...
        <div className="min-h-screen bg-[#F4F5F7] font-sans flex flex-col">
            ...
            <div className="bg-[#1C2A59] pt-28 pb-14 px-4 lg:px-8">
                <div className="max-w-7xl mx-auto">
                    ...
                    <h1 className="text-3xl md:text-4xl font-extrabold text-white mb-2">My Lost Items</h1>
                    <p className="text-gray-300 font-medium">Reports you created for lost items.</p>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 flex-1 w-full">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-24">
                        ...
                        <p className="text-sm text-[#3E4A56] font-medium">Loading your lost item reports...</p>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 p-16 text-center shadow-sm">
                        ...
                        <h3 className="text-[#1C2A59] font-bold text-xl mb-2">
                            {items.length === 0 ? 'No lost item reports yet' : 'No items match your filters'}
                        </h3>
                        <p className="text-gray-500 text-sm mb-6">
                            {items.length === 0 ? 'Create a lost item report to see it here.' : 'Try adjusting your search criteria or clear the filters to see all your reports.'}
                        </p>
                        {items.length === 0 ? (
                            <Link href="/lost-items/new" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold" style={{ backgroundColor: '#F0A500', color: '#1C2A59' }}>
                                Report Lost Item
                            </Link>
                        ) : (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredItems.map(item => (
                            <div key={item._id} className="flex flex-col gap-3">
                                <ItemCard item={item} type="lost" showEditCountdown={false} />
