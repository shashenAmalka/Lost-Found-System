                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                    ...
                    <div className="md:col-span-3 space-y-5">
                        <div className="flex flex-wrap items-start gap-3 justify-between pb-3 border-b border-gray-200">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2 mt-3 block">
                                    ...
                                    <span className="text-xs px-2 py-1 rounded font-bold" style={{ backgroundColor: '#fee2e2', color: '#ef4444', border: '1px solid #fca5a5' }}>🔍 Lost</span>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { icon: MapPin, label: 'Possible Location', value: item.possibleLocation },
                        ...
                        {(item.color || item.brand || item.uniqueIdentifier) && (
                            <div className="bg-white rounded border border-gray-200 p-5 shadow-sm space-y-4">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm bg-gray-50 p-3 rounded border border-gray-100">
                                    ...
                                    {item.uniqueIdentifier && <div><span className="text-gray-500 text-xs font-medium block mb-1">Identifier:</span><span className="text-[#1C2A59] font-bold">{item.uniqueIdentifier}</span></div>}
                                </div>
                            </div>
                        )}
                        ...
                        <div className="bg-[#1C2A59] rounded p-4 flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-white text-sm font-bold">{item.postedByName || 'Anonymous User'}</div>
                                    <div className="text-indigo-200 text-xs mt-0.5">Reported missing on {new Date(item.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                                </div>
                            </div>
                            <div className="text-right">
                                <Link href="/found-items/new" className="text-xs font-bold text-white underline hover:text-[#F0A500]">Did you find this?</Link>
                            </div>
                        </div>
                    </div>
                </div>
